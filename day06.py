...
# def get_all_guard_states(
#     guard_map: list[str], 
#     current_coords: tuple[int, int], 
#     current_orientation: str, 
#     guard_states: tuple[tuple[tuple[int, int], str]] | tuple = ()
#     ) -> tuple[tuple[tuple[int, int], str]]:

#     updated_guard_states: tuple[tuple[tuple[int, int], str]] = tuple( list(guard_states) + [ (current_coords, current_orientation) ] )
#     next_move: str = get_next_move(guard_map, current_orientation, current_coords)
#     if next_move == 'exit':
#         return updated_guard_states
#     next_orientation: str = next_move
#     next_coords: tuple[int, int] = get_next_coords(guard_map, next_orientation, current_coords)
#     return get_all_guard_states(guard_map, next_coords, next_orientation, updated_guard_states)

# all_guard_states: tuple[tuple[tuple[int, int], str]] = get_all_guard_states(guard_map, (initial_row, initial_column), initial_orientation)
...
# def obstacle_creates_loop(guard_map: list[str], index: int ) -> bool:
#     obstacle_coords: tuple[int, int] = all_guard_states[index + 1][0]
#     if index % 10 == 0:
#         guard_state_count: int = len(all_guard_states)
#         percentage: int = round(index / (guard_state_count - 1) * 100)
#         print('\r', end='')
#         print(f'Processed {index} ({percentage}%) of {guard_state_count} locations. Current location: {obstacle_coords}', end = '\r')
#     updated_guard_map: list[str] = get_updated_guard_map(guard_map, obstacle_coords)
#     current_state: tuple[tuple[int, int], str] = all_guard_states[index]
...
# def get_loop_causing_obstacle_coords(guard_map: list[str]) -> set[tuple[int, int]]:
#     loop_causing_obstacle_coords: set[tuple[int, int]] = set([ #type: ignore[var-annotated]
#         get_next_coords(guard_map, guard_state[1], guard_state[0]) 
#         for index, guard_state in enumerate(all_guard_states[:-1]) 
#         if obstacle_creates_loop(guard_map, index)
#     ])
#     return loop_causing_obstacle_coords

def get_coords_at_which_obstacle_creates_loop(index: int) -> tuple[int, int] | bool:
    obstacle_coords: tuple[int, int] = potential_obstacle_locations[index]#all_guard_states[index + 1][0]
    if obstacle_coords == (initial_row, initial_column):
        return False
    updated_guard_map: list[str] = get_updated_guard_map(guard_map, obstacle_coords)
    current_state: tuple[tuple[int, int], str] = ((initial_row, initial_column), initial_orientation)#all_guard_states[index]
    previous_states: set[tuple[tuple[int, int], str]] = set()
    while current_state not in previous_states:
        previous_states = set( list(previous_states) + [current_state] )
        current_coords, current_orientation = current_state[0], current_state[1]
...
coords_at_which_obstacle_creates_loop: list[tuple[int,int] | bool] = pool.map(
